...
let sourceFolder = "assets/maps";
let outputPath = "derived/maps.h";

let maps = dir(sourceFolder)
    .filter( file=>/\.tmx$/i.test(file) )
    .reduce( (index, file) => {
        let xml = XML(read(`${sourceFolder}/${file}`));
        ...
        promises.push(...getTileSets(xml).map(ts=>{
            ...
            let imgPath = sourceFolder + path.sep + ts.source;
