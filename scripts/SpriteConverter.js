...
let sourceFolder = "assets/sprites";
let outputDataFolderName = "sprites";
let outputDataFolder = "derived/" + outputDataFolderName;
let outputIncludePath = "derived/sprites.h";
let outputCPPPath = "derived/sprites.cpp";

function start(){
    let sprites = (dir(sourceFolder)||[])
    .filter( file=>/\.json$/i.test(file) )
    .reduce( (index, file) => {
        let sprite = JSON.parse(read(`${sourceFolder}/${file}`));
        ...
        promises.push(
            readImage(`${sourceFolder}/${sprite.meta.image}`)
            .then( image => {
                ...
                head[0] = sum & 0xFF;
                head[1] = sum >> 8;
                sprite.bin = `${outputDataFolderName}/${name}.bin`;
                write(`${outputDataFolder}/${name}.bin`, new Uint8Array(head.concat(acc)));
                return sprite;
            }));
